...
const FLYING_DOCS = [
  { id: 1, label: 'Engineering', color: '#26a7e0', icon: '⚙️' },
  { id: 2, label: 'Bookkeeping & Invoices', color: '#10b981', icon: '💵' },
  { id: 3, label: 'Students', color: '#8b5cf6', icon: '🎓' },
  { id: 4, label: 'Inventory', color: '#84cc16', icon: '📦' },
];
...
const MESSY_PAPERS = [
  { rotate: -25, x: -20, y: -22, color: '#ef4444', label: 'urgent' },
  { rotate: 20, x: 18, y: -14, color: '#f59e0b' },
  { rotate: -10, x: -8, y: 6, color: '#8b5cf6', label: 'draft' },
  { rotate: 14, x: 14, y: 14, color: '#10b981' },
];
...
function FlyingPaper({ doc, index, cycleKey, onArrived }) {
  ...
  return (
    <div
      key={`${cycleKey}-${doc.id}`}
      className="absolute z-20 top-0 bottom-0 flex items-center justify-center opacity-0 animate-fly-doc w-max"
    ...
    >
      ...
      <div className="rounded-lg md:rounded-xl overflow-hidden flex items-center gap-0 min-w-[110px] md:min-w-[200px] backdrop-blur-sm"
        style={{
      ...
      >
        ...
        <div className="flex items-center gap-1.5 md:gap-2.5 px-2 md:px-3.5 py-1.5 md:py-2.5">
          ...
          <span className="text-[7px] md:text-xs font-extrabold text-slate-800 uppercase tracking-wider whitespace-nowrap">
            {doc.label}
          </span>
        </div>
      </div>
    ...
    <div className="w-full py-1 flex flex-col items-center justify-center space-y-1 md:space-y-4 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center space-y-1 md:space-y-3 z-20 mb-2 md:mb-12">
          ...
          <p className="text-[11px] md:text-xl font-bold text-slate-500 max-w-2xl text-center leading-relaxed px-4 md:px-0">
            Streamline your business operations with Baava Tech's expert back-office support
          </p>
        </div>
      ...
      <div className="w-full flex items-center justify-between max-w-5xl relative px-1 md:px-8 z-10">
        ...
        <div className="flex flex-col items-center space-y-1 md:space-y-2 relative z-20 flex-shrink-0">
          <div className="relative w-[65px] h-[75px] md:w-[155px] md:h-[165px] flex items-center justify-center">
        ...
        {/* MIDDLE: Pipeline */}
        <div className="flex-1 h-12 md:h-32 relative flex items-center justify-center z-10 mx-1 md:mx-6">
        ...
        <div className="flex flex-col items-center space-y-1 md:space-y-2 relative z-50 flex-shrink-0">
          <div className="relative">
            ...
            <motion.div
              ...
              className="relative w-[75px] h-[100px] md:w-[165px] md:h-[220px] rounded-xl md:rounded-2xl shadow-2xl flex flex-col items-center border overflow-hidden transition-colors duration-700"
              style={{
            ...
            >
              ...
              <div className="flex-1 w-full px-1.5 md:px-2 py-1 flex flex-col gap-[2px] md:gap-[3px] items-center overflow-hidden">
                <AnimatePresence>
                  {FLYING_DOCS.filter(d => arrivedIds.has(d.id)).map(doc => (
                    <motion.div key={doc.id}
                      initial={{ opacity: 0, scale: 0.3, x: -30, y: 10 }}
                      animate={{ opacity: 1, scale: 1, x: 0, y: 0 }}
                      transition={{ type: "spring", stiffness: 150, damping: 14 }}
                      className="flex items-center gap-1 md:gap-1.5 px-1 md:px-2 py-[2px] md:py-[3px] rounded md:rounded-lg text-[4.5px] md:text-[8px] font-bold text-white uppercase tracking-wider w-full justify-center shadow-sm"
                    ...
                    >
                      ...
                      <span className="drop-shadow-sm truncate">{doc.label}</span>
                    </motion.div>
                  ))}
                </AnimatePresence>
                {arrivedIds.size === 0 && (
                  <div className="flex-1 flex flex-col items-center justify-center gap-0.5 md:gap-1 opacity-40">
                    <Database className="w-5 h-5 md:w-11 md:h-11 text-white/50" />
                    <span className="text-[4px] md:text-[7px] text-white/40 uppercase tracking-widest font-mono">Awaiting</span>
                  </div>
                )}
              </div>
          ...
          <div className="text-center">
            <span className="text-[8px] md:text-xs uppercase tracking-[0.2em] text-[#00639b] font-extrabold font-mono">Baava Tech Portal</span>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 }}
        className="flex items-center justify-center gap-x-1.5 gap-y-1 md:gap-5 z-10 flex-wrap mt-2 md:mt-0 w-full md:max-w-none mx-auto">
